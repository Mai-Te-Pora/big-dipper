...
export default class Activites extends Component {
    ...
    render(){
        ...
        case "cosmos-sdk/MsgWithdrawValidatorCommission":
            return <p><Account address={msg.value.validator_address} /> {(this.props.invalid)?<T>activities.failedTo</T>:''}<MsgType type={msg.type} /><T> {(!this.props.invalid)?<T _purify={false} amount={new Coin(parseInt(events['withdraw_commission'][0].value)?parseInt(events['withdraw_commission'][0].value):0, events['withdraw_commission'][0].value?events['withdraw_commission'][0].value.replace(/[0-9]/g, ''):events['withdraw_commission'][0].value).toString(8)}>activities.ofAmount</T>:''}common.fullStop</T></p>
        case "cosmos-sdk/MsgWithdrawDelegationReward":
            return <p><Account address={msg.value.delegator_address}/> {(this.props.invalid)?<T>activities.failedTo</T>:''}<MsgType type={msg.type} /> {(!this.props.invalid)?<T _purify={false} amount={new Coin(parseInt(events['withdraw_commission'][0].value)?parseInt(events['withdraw_commission'][0].value):0, events['withdraw_rewards'][0].value?events['withdraw_rewards'][0].value.replace(/[0-9]/g, ''):events['withdraw_rewards'][0].value).toString(8)}>activities.ofAmount</T>:''} <T>activities.from</T> <Account address={msg.value.validator_address} /><T>common.fullStop</T></p>
