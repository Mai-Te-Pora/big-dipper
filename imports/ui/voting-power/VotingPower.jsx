    ...
    componentDidUpdate(prevProps){
        if (prevProps.stats != this.props.stats){
            ...
            for (let i in this.props.stats){
                totalVotingPower += this.props.stats[i].voting_power/100;
                if (i > 0){
                    accumulatePower[i] = accumulatePower[i-1] + this.props.stats[i].voting_power/100;
                }
                else{
                    accumulatePower[i] = this.props.stats[i].voting_power/100;
                }
            }

            for (let v in this.props.stats){
                labels.push(this.props.stats[v].description?this.props.stats[v].description.moniker:'');
                data.push(this.props.stats[v].voting_power/100);
                let alpha = (this.props.stats.length+1-v)/this.props.stats.length*0.8+0.2;
                backgroundColors.push('rgba(6, 60, 79,'+alpha+')');
            }
            this.setState({
                data:{
                    labels:labels,
