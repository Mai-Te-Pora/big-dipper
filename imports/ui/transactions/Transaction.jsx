      ...
      if (this.props.transactionExist) {
        ...
        return (
          <Container id="transaction">
            ...
            <h4>
              <T>transactions.transaction</T> {!tx.code ? <TxIcon valid /> : <TxIcon />}
            </h4>
            {tx.code ? (
              <Row>
                <Col xs={{ size: 12, order: "last" }} className="error">
                  <Alert color="danger">
                    <CosmosErrors code={tx.code} codespace={tx.codespace} log={tx.raw_log} />
                  </Alert>
                </Col>
              </Row>
            ) : (
            ...
            <Card>
              ...
              <CardBody>
                <Row>
                      ...
                      tx.tx.value.fee.amount.map((fee, i) => {
                        return (
                          <span className="text-nowrap" key={i}>
                            {" "}
                            {fee.amount / Meteor.settings.public.stakingFraction >= 1
                              ? new Coin(parseFloat(fee.amount), fee.denom).toString(0)
                  ...
                  <Col md={8} className="value">
                    {numbro(tx.gas_used).format("0,0")} / {numbro(tx.gas_wanted).format("0,0")}
                  </Col>
                  <Col md={4} className="label">
            ...
            {tx.tx.value.msg && tx.tx.value.msg.length > 0
              ? tx.tx.value.msg.map((msg, i) => {
                  return (
                    <Card body key={i}>
                      <Activities msg={msg} invalid={!!tx.code} events={tx.logs && tx.logs[i] ? tx.logs[i].events : null} denom={this.denom} />
                    </Card>
                  );
                })
              : ""}
