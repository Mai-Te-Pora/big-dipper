...
const ValidatorRow = (props) => {
    ...
    return <Card body>
        <SentryBoundary><Row className="validator-info">
            ...
            <Col className="voting-power data" xs={{size:8, offset:2}} md={{size:3, offset:0}} lg={2}><i className="material-icons d-md-none">power</i>  <span>{props.validator.voting_power?numbro(props.validator.voting_power/100).format('0,0'):0} ({props.validator.voting_power?numbro(props.validator.voting_power/props.totalPower).format('0.00%'):"0.00%"})</span></Col>
            <Col className="self-delegation data" xs={{size:6,offset:0}} md={{size:1,offset:0}}><i className="material-icons d-sm-none">equalizer</i> <span>{props.validator.self_delegation?numbro(Math.round(props.validator.self_delegation*props.validator.voting_power/100)).format('0,0'):'N/A'}</span></Col>
            ...
            {(props.inactive)?<Col className="last-seen data" xs={{size:10,offset:0}}md={{size:3, offset:0}}>{props.validator.lastSeen?<TimeStamp time={props.validator.lastSeen}/>:''}</Col>:''}
