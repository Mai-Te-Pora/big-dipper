...
export default class Coin {
  ...
  toString(precision) {
    ...
    let minStake =
      Coin.StakingCoin.fraction / (precision ? Math.pow(10, precision) : 10000);
  ...
  toStringPool(precision) {
    ...
    if (this.amount < minStake) {
      ...
      return `${
        precision
          ? numbro(this.stakingAmount).format("0,0." + "0".repeat(precision))
          : autoformatPool(this.stakingAmount)
      } ${this._coin.displayName}`;
    }
  }
  ...
  mintString(formatter) {
    ...
    let denom =
      this._coin == "" ? Coin.StakingCoin.displayName : this._coin.denom;
    return `${amount} ${denom}`;
  }
  ...
  stakeString(formatter) {
    ...
    return `${numbro(amount).format("0,0.00000000")} ${
